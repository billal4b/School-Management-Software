                    ...
                    if (isset($ExamSubHeads)) {
                        foreach ($ExamSubHeads as $key2 => $value2) {
                            ...
                            if ($SearchKey2) {
                                ...
                                $PassMarks = $ActiveExamSubHeads[$SearchKey2]['PassMarks'] ? $ActiveExamSubHeads[$SearchKey2]['PassMarks'] : 0;
...
//                                    $IsMarksInput = $ExistingMarksInputs[$SearchKey]['IsMarksInput'];
                                    $IsAbsent = $ExistingMarksInputs[$SearchKey]['IsAbsent'];
                                }
                                ?>
                                <td>
                                                ...
                                                <div class="col-sm-4">
                                                    ...
                                                    if ($MarksInputID > 0) {
                                                        ...
                                                        if (in_array(Auth::user()->role_id, $RolesID)) {
                                                            echo '<input type="hidden" name="MaxMarks" id="MaxMarks" value="' . $MaxMarks . '">';
                                                            echo '<input type="text" class="form-control Marks" id="Marks" name="Marks" value="' . $Marks . '">';
                                                            echo '<input type="hidden" id="PassMarks" name="PassMarks" value="' . $PassMarks . '">';
                                                            //echo '<input type="hidden" name="IsMarksInput" id="IsMarksInput" value="' . $IsMarksInput . '">';
                                                        } else {
                                                            echo '<p class="form-control-static">' . ($IsAbsent == 1 ? 'ABSENT' : $Marks) . '</p>';
                                                        }
                                                    } else {
                                                        //echo '<input type="hidden" id="MaxMarks" value="' . $MaxMarks . '">';
                                                        ?>
                                                    <input type="hidden" name="MaxMarks" id="MaxMarks" value="<?php echo $MaxMarks; ?>">
                                                        <input type="text" class="form-control Marks" id="Marks" name="Marks" value="<?php echo $Marks; ?>">
                                                        
                                                        <input type="hidden" name="PassMarks" id="PassMarks" value="<?php echo $PassMarks; ?>">
                                                        <!--<input type="hidden" name="IsMarksInput" id="IsMarksInput" value="<?php //echo $IsMarksInput; ?>">-->
                                                        <?php
                                                    }
                                                    ?>
...
//                                                    echo $Marks < 0 ? 'Absent' : $Marks;
                                                    if (in_array(Auth::user()->role_id, $RolesID)) {
                                                        ...
                                                        <div class="col-sm-6">
                                                            <div class="checkbox checkbox-danger">
                                                                <input type="checkbox" id="Absent<?php echo $IbvID . '-' . $ClassID . '-' . $GroupID . '-' . $ExamTypeID . '-' . $ExamHead . '-' . $value2->exam_sub_head_id . '-' . $SubjectID . '-' . $value->student_id; ?>" value="0" <?php echo $IsAbsent == 1 ? 'checked' : ''; ?> class="Absent" name="Absent">
                                                                <label for="Absent<?php echo $IbvID . '-' . $ClassID . '-' . $GroupID . '-' . $ExamTypeID . '-' . $ExamHead . '-' . $value2->exam_sub_head_id . '-' . $SubjectID . '-' . $value->student_id; ?>">
                                                                    Absent
                                                                </label>
                                                            </div>
...
//            alert(MaxMarks);return;
            if (parseFloat(MaxMarks) < parseFloat($(this).val())) {
                alert('Limit Access!');
                $(this).val(0);
            }
//            var input = $(this).next().next().find('input#IsMarksInput');
//            //console.log(input);
//            input.attr('value', '1');
        });
...
//$('#ShowMarksInput').html(response);
        ...
        $(document).on('click', '.Absent', function () {
            ...
            var input = $(this).parent().parent().prev().find('input#Marks');
//            console.log(input.attr('id'));return;
